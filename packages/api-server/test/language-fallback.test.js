...
test('API Server - Language Fallback', async () => {
  ...
  const resDe = await fetch('http://localhost:3001/api/projects/silly-eyes');
  ...
  const resEn = await fetch('http://localhost:3001/api/projects/scratchpc-interactive-book');
  const dataEn = await resEn.json();
  assert.strictEqual(dataEn.data.attributes.content.title, 'Make an interactive book');
  
  // Test explicit language request
  const resExplicit = await fetch('http://localhost:3001/api/projects/silly-eyes?lang=en');
