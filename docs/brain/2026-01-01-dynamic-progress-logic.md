...
2.  **Step Completion**:
    ...
    *   A step without tasks is considered complete by default.
3.  **Project Completion**: A project is 100% complete only if all its steps are complete (dynamically calculated).
4.  **Resuming**:
...
5.  **Tracking**: `step_complete` events are still tracked for history/dashboard visibility but are no longer used for progress calculation.
